...
private:
  ...
  // One cublasGemmGroupedBatchedEx over all (fp16) expert GEMMs: group g computes C[g] = A[g]^T .
  // B[g] with A [m,k] row-major (OP_T), B [n_g,k] row-major, C [n_g,m] row-major. Collapses the ~64
...
private:
  ...
  ExecCtx decode_ctx() const {
    return ExecCtx{slot_ptr_.data(), 1, true};
  }
  ...
  bool k_eq_v(int layer) const {
    return cfg_.attention_k_eq_v && cfg_.layer_full[layer];
  }
  bool has_ple() const {
    return cfg_.hidden_size_per_layer_input > 0;
  }  // E2B: yes, 12B: no
  ...
  struct QuantWeight {
    ...
    bool host =
        false;  // true = packed/scales live on host (expert streaming); stage_moe H2D's them
  };
  ...
  // Sequence-prefill routing: one [top_k] slot per token ([max_tokens][top_k]). The MoE ops loop
  // the T==1 kernels per token in seq mode (DeepSeek batched prefill), reading token t's slot.
  ...
  std::int8_t* d_moe_actq_ =
      nullptr;                    // [P, H] int8-quantized grouped activations (int4-direct path)
  ...
  // grouped_gemm_ex scalar-array scratch. transa/transb held as int (cublasOperation_t is
  // int-backed; cast at the call site) to keep cublas out of this header.
