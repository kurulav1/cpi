...
// DeepSeek-V2/V3 Multi-head Latent Attention, matching transformers' DeepseekV2Attention exactly
// (the real V2-Lite structure, verified in deepseek_mla_real_test against an HF dump). Differs from
// the generic mla_forward.hpp brick: (1) no q-lora; q is a direct projection; (2) a SINGLE
// kv_a_proj_with_mqa produces [latent(kv_lora) | k_pe(qk_rope)] and the k_pe head is shared across
// all heads; (3) kv_a_layernorm (RMSNorm) on the latent before kv_b up-projection; (4)
// INTERLEAVED-complex RoPE (pairs (2p,2p+1)) using the model's inv_freq, with the rotated vectors
// scaled by attention_scaling (the YARN mscale, baked into the rope, not the softmax scale which
// stays 1/sqrt(hd)).
//
// fp32, naive-reconstruction prefill. inv_freq + attention_scaling are supplied by the caller
// (dumped from the model), so this op doesn't re-derive YARN; computing inv_freq from config is a
// separate step.
...
struct DSMLADims {
  ...
  }  // per-head query/key dim
  int kvh() const {
    return qk_nope + v_head;
  }  // per-head [k_nope | v] from kv_b
  int kva() const {
    return kv_lora + qk_rope;
  }  // kv_a_proj output width
};
...
// One thread per (query i, head h). q is [T, nh*qkhd] (nope|rope inline, rope already rotated); kv
// is [T, nh*(qk_nope+v_head)] (per head [k_nope | v]); kpe is [T, qk_rope] (rotated, shared).
