...
// runs ONE real GEMM per expert (the weight is streamed once, reused across its tokens) and scatters
// the weighted results back. These are the two glue kernels; the routing is host-computed (T*top_k
// is tiny) and the per-expert GEMMs reuse the verified dequant + cuBLAS path.
//
//   gather:  gathered[p] = src[perm[p]]                    (collect an expert's token rows)
//   scatter: out_f32[perm[p]] += rweight[p] * grouped[p]   (weighted, atomic; tokens repeat top_k x)
...
__global__ inline void moe_scatter_accum_kernel(const __half* grouped, const int* perm,
                                                const float* rweight, float* out_f32, int H, int P) {
  const std::size_t idx = static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  const std::size_t total = static_cast<std::size_t>(P) * H;
  if (idx >= total) return;
  const int p = static_cast<int>(idx / H);
...
inline void launch_moe_scatter_accum(const __half* grouped, const int* perm, const float* rweight,
  ...
  const int block = 256;
  const std::size_t grid = (total + block - 1) / block;
  moe_scatter_accum_kernel<<<static_cast<unsigned>(grid), block, 0, stream>>>(grouped, perm, rweight,
                                                                              out_f32, H, P);
}
...
inline void launch_moe_f32_to_f16(const float* in, __half* out, std::size_t n, cudaStream_t stream) {
  const int block = 256;
