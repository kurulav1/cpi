...
//   * tp (tensor): within a layer, the dense attention+MLP matrices and the KV heads are split tp ways;
...
inline const char* quant_name(Quant q) {
  switch (q) {
    case Quant::FP16: return "fp16";
    case Quant::INT8: return "int8";
    case Quant::INT4: return "int4";
  }
  return "?";
}
...
inline double quant_bytes_per_elem(Quant q) {
  switch (q) {
    case Quant::FP16: return 2.0;
    case Quant::INT8: return 1.0;
...
struct ModelDims {
  ...
  int q_hidden() const { return num_heads * head_dim; }
  int kv_hidden() const { return num_kv_heads * head_dim; }
  bool is_moe() const { return num_experts > 0 && expert_intermediate > 0; }
};
...
struct PlanConfig {
  ...
  int world() const { return tp * pp * ep; }
};
...
struct Footprint {
  ...
  std::size_t weights() const { return embedding + head + attn + dense_mlp + experts; }
  std::size_t total() const { return weights() + kv_cache + activations; }
};
...
inline std::size_t matrix_bytes(long long rows, long long cols, Quant q) {
  ...
  switch (q) {
    case Quant::FP16: return r * c * 2;
    case Quant::INT8: return r * c + r * sizeof(float);
    case Quant::INT4: return r * ((c + 1) / 2) + r * sizeof(float);
  }
  return r * c * 2;
}
// Ceil-divide, for balanced shard sizing (the heaviest rank owns ceil(n/parts)).
inline int ceil_div(int n, int parts) { return parts > 0 ? (n + parts - 1) / parts : n; }
}  // namespace detail
...
inline Footprint estimate_footprint(const ModelDims& d, const PlanConfig& pc) {
  ...
  // --- activations: coarse working-set (a handful of hidden-sized fp16 buffers over the batch x seq
  // token span). Labelled approximate; it is workload-dependent, not a fixed allocation. ---
...
// Scan tp/pp/ep over powers of two up to `max_degree` and return the feasible configs (heaviest rank
