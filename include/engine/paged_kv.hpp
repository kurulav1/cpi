...
class BlockAllocator {
 public:
  ...
  int used_count() const { return num_blocks_ - free_count(); }
...
class SequenceBlockTable {
 public:
  ...
  int offset_for(int pos) const { return pos % block_size_; }
  ...
  int length_tokens() const { return length_tokens_; }
 ...
 private:
  ...
  int shared_prefix_blocks_ = 0;  // leading blocks that are shared (refcounted), not owned-exclusive
