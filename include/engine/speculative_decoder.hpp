...
struct SpeculativeStats {
  ...
  double accept_rate() const { return drafted > 0 ? static_cast<double>(accepted) / drafted : 0.0; }
  double tokens_per_round() const { return rounds > 0 ? static_cast<double>(emitted) / rounds : 0.0; }
};
