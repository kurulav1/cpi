...
The `--out-dir` from the first step is an intermediate: it holds every tensor as a
separate `.bin` and is as large as the model itself, so a 7B costs ~28 GB on disk
rather than ~14 until you delete it. Once the `.ll2c` is packed and validated,
`model_bins` can go.
