...
"C:\Program Files\NVIDIA Corporation\Nsight Compute 2026.1.0\target\windows-desktop-win7-x64\ncu.exe" --kernel-name regex:%1 --launch-skip %2 --launch-count %3 --section SpeedOfLight --section WarpStateStats --section SchedulerStats build-cuda-ninja\cpi.exe %M%\gemma4-e2b.cpi --tokenizer %M%\hf\tokenizer.json --chat-template gemma --prompt "Write one sentence about the sea." --max-new 6 --temp 0 --weight-quant int4
