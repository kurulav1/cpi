...
void LlamaEngine::resident_projection_half(const void* w, const void* x, void* y, int out_features,
                                           int in_features, int warps_per_block, int tile_pairs,
                                           int rows_per_warp) {
  kernels::launch_rowmajor_half_gemv_f16(
      static_cast<const __half*>(w), static_cast<const __half*>(x), static_cast<__half*>(y),
...
void LlamaEngine::resident_int8_mlp_w13(const LayerDeviceInt8Weights& lw_i8, int inter,
  ...
  if (lw_i8.mlp_int4) {
    kernels::launch_weight_only_int4_matvec_dual_dp4a(
        ...
        static_cast<__half*>(d_ff1_), static_cast<__half*>(d_ff2_), inter, hidden, compute_stream_,
        resident_mlp_w13_warps_, resident_mlp_w13_tile_packed4_, resident_mlp_w13_warps_per_row_);
    return;
  }
  kernels::launch_weight_only_int8_matvec_dual_dp4a(
      lw_i8.w1, lw_i8.s_w1, lw_i8.w3, lw_i8.s_w3, d_prefill_i8_, d_prefill_i8_scales_,
      static_cast<__half*>(d_ff1_), static_cast<__half*>(d_ff2_), inter, hidden, compute_stream_,
      resident_mlp_w13_warps_, resident_mlp_w13_tile_packed4_, resident_mlp_w13_warps_per_row_);
}
...
void LlamaEngine::resident_int8_mlp_w2(const LayerDeviceInt8Weights& lw_i8, int hidden, int inter) {
  if (lw_i8.mlp_int4) {
    kernels::launch_weight_only_int4_matvec_dp4a(
        ...
        hidden, inter, compute_stream_, resident_mlp_w2_warps_, resident_mlp_w2_tile_packed4_,
...
void LlamaEngine::tune_resident_projection_backends() {
  ...
  const std::vector<ProjectionConfig> qkv_configs = {
      {8, 128, 1},  {8, 128, 2},  {8, 256, 1}, {8, 256, 2}, {16, 128, 1}, {16, 128, 2},
      {16, 256, 1}, {16, 256, 2}, {8, 512, 1}, {8, 512, 2}, {16, 512, 1}, {16, 512, 2}};
  const std::vector<ProjectionConfig> wo_configs = {
      {4, 128, 1}, {4, 128, 2}, {4, 256, 1}, {4, 256, 2}, {8, 128, 1},
      {8, 128, 2}, {8, 256, 1}, {8, 256, 2}, {8, 512, 1}, {8, 512, 2}};
  const std::vector<ProjectionConfig> lm_configs = {
      {8, 128, 1}, {8, 128, 2}, {8, 256, 1}, {16, 128, 1}, {8, 512, 1}, {8, 512, 2}, {16, 512, 1}};
  const std::vector<ProjectionConfig> mlp_w13_configs = {{8, 256, 2},  {8, 256, 1}, {16, 256, 2},
                                                         {16, 256, 1}, {8, 512, 2}, {8, 512, 1},
                                                         {8, 128, 2},  {8, 128, 1}};
  const std::vector<ProjectionConfig> mlp_w2_configs = {{8, 256, 2},  {8, 256, 1}, {4, 256, 1},
                                                        {8, 128, 2},  {8, 128, 1}, {4, 128, 1},
                                                        {16, 128, 2}, {16, 128, 1}};

  // Skip fp16 QKV/wo tuning when they've been freed (int8 proj always active for GPU-cached
  // layers).
    ...
    for (const auto& cfg_candidate : qkv_configs) {
      const double ms = benchmark_cuda_launch(compute_stream_, warmup, iters, [&] {
        resident_projection_half(lw.wqkv, d_x_norm_, d_q_, q_hidden + 2 * kv_hidden, hidden,
                                 cfg_candidate.warps, cfg_candidate.tile,
  ...
  if (!cached_int8_proj_enabled_ && lw.wo) {
    ...
    for (const auto& cfg_candidate : wo_configs) {
      const double ms = benchmark_cuda_launch(compute_stream_, warmup, iters, [&] {
        resident_projection_half(lw.wo, d_att_, d_ff3_, hidden, q_hidden, cfg_candidate.warps,
                                 cfg_candidate.tile, cfg_candidate.warps_per_row);
      });
  ...
  const double lm_cublas_ms = benchmark_cuda_launch(compute_stream_, warmup, iters, [&] {
    detail::dispatch_linear_rowmajor_weight(
        cublas_, cublas_lt_, &lt_plan_cache_, lt_workspace_, lt_workspace_bytes_, compute_stream_,
        d_lm_head_, d_x_norm_, d_logits_, cfg.vocab_size, hidden, 1, CUDA_R_32F);
  });
  ...
  bool have_mlp_int8_tuning = cached_int8_mlp_enabled_ && !layer_cache_i8_.empty() &&
                              layer_cache_i8_.front().w1 && layer_cache_i8_.front().w2 &&
                              layer_cache_i8_.front().w3;
  if (have_mlp_int8_tuning) {
    ...
    kernels::launch_quantize_rowwise_fp16_to_int8(static_cast<const __half*>(d_x_norm_),
                                                  d_prefill_i8_, d_prefill_i8_scales_, 1, hidden,
                                                  compute_stream_);
    ...
    for (const auto& cfg_candidate : mlp_w13_configs) {
      const double ms = benchmark_cuda_launch(compute_stream_, warmup, iters, [&] {
        kernels::launch_weight_only_int8_matvec_dual_dp4a(
            lw_i8.w1, lw_i8.s_w1, lw_i8.w3, lw_i8.s_w3, d_prefill_i8_, d_prefill_i8_scales_,
            static_cast<__half*>(d_ff1_), static_cast<__half*>(d_ff2_), cfg.intermediate_size,
            hidden, compute_stream_, cfg_candidate.warps, cfg_candidate.tile,
            cfg_candidate.warps_per_row);
      });
    ...
    kernels::launch_quantize_rowwise_fp16_to_int8(static_cast<const __half*>(d_ff2_), d_prefill_i8_,
                                                  d_prefill_i8_scales_, 1, cfg.intermediate_size,
                                                  compute_stream_);
    CUDA_CHECK(cudaStreamSynchronize(compute_stream_));
    ...
    for (const auto& cfg_candidate : mlp_w2_configs) {
      const double ms = benchmark_cuda_launch(compute_stream_, warmup, iters, [&] {
        kernels::launch_weight_only_int8_matvec_dp4a(
            lw_i8.w2, lw_i8.s_w2, d_prefill_i8_, d_prefill_i8_scales_, static_cast<__half*>(d_ff3_),
            hidden, cfg.intermediate_size, compute_stream_, cfg_candidate.warps, cfg_candidate.tile,
            cfg_candidate.warps_per_row);
      });
      ...
      const double ms = benchmark_cuda_launch(compute_stream_, warmup, iters, [&] {
        if (lw_i8.proj_int4) {
          kernels::launch_weight_only_int4_matvec_dp4a(
              ...
              q_hidden + 2 * kv_hidden, hidden, compute_stream_, cfg_candidate.warps,
              cfg_candidate.tile, cfg_candidate.warps_per_row);
        } else {
          kernels::launch_weight_only_int8_matvec_dp4a(
              lw_i8.wqkv, lw_i8.s_wqkv, static_cast<const std::int8_t*>(d_prefill_i8_),
      ...
      const double ms = benchmark_cuda_launch(compute_stream_, warmup, iters, [&] {
        if (lw_i8.proj_int4) {
          kernels::launch_weight_only_int4_matvec_dp4a(
              ...
              q_hidden, compute_stream_, cfg_candidate.warps, cfg_candidate.tile,
              cfg_candidate.warps_per_row);
        } else {
          kernels::launch_weight_only_int8_matvec_dp4a(
              lw_i8.wo, lw_i8.s_wo, static_cast<const std::int8_t*>(d_prefill_i8_),
  ...
  if (options_.verbose) {
    const bool proj_is_int4 =
        cached_int8_proj_enabled_ && !layer_cache_i8_.empty() && layer_cache_i8_.front().proj_int4;
    const char* proj_mode = proj_is_int4 ? "int4" : "int8";
    std::cout << std::fixed << std::setprecision(3) << "[engine] resident_projection_backend"
              << " qkv="
              << (cached_int8_proj_enabled_ ? proj_mode
                                            : (resident_custom_qkv_ ? "custom" : "cublaslt"))
              << " wo="
              << (cached_int8_proj_enabled_ ? proj_mode
                                            : (resident_custom_wo_ ? "custom" : "cublaslt"))
    ...
    if (!cached_int8_proj_enabled_) {
      std::cout << " qkv_ms=" << qkv_cublas_ms << "/" << qkv_custom_ms
                ...
                << resident_qkv_rows_per_warp_ << " wo_ms=" << wo_cublas_ms << "/" << wo_custom_ms
    ...
    if (have_mlp_int8_tuning) {
      std::cout << " mlp_w13_ms=" << mlp_w13_best_ms << " mlp_w13_cfg=" << resident_mlp_w13_warps_
                << "x" << resident_mlp_w13_tile_packed4_ << "@" << resident_mlp_w13_warps_per_row_
                << " mlp_w2_ms=" << mlp_w2_best_ms << " mlp_w2_cfg=" << resident_mlp_w2_warps_
                << "x" << resident_mlp_w2_tile_packed4_ << "@" << resident_mlp_w2_warps_per_row_;
    }
    if (have_proj_int8_tuning) {
      std::cout << " " << proj_mode << "_qkv_ms=" << int8_qkv_best_ms << " " << proj_mode
                << "_qkv_cfg=" << resident_int8_qkv_warps_ << "x" << resident_int8_qkv_tile_packed4_
                << "@" << resident_int8_qkv_warps_per_row_ << " " << proj_mode
                << "_wo_ms=" << int8_wo_best_ms << " " << proj_mode
                << "_wo_cfg=" << resident_int8_wo_warps_ << "x" << resident_int8_wo_tile_packed4_
