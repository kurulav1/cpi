...
void Llama4CudaEngine::allocate_runtime_buffers() {
  ...
  const std::size_t half_bytes_hidden = static_cast<std::size_t>(hidden_) * sizeof(__half);
  const std::size_t half_bytes_kv = static_cast<std::size_t>(kv_dim_) * sizeof(__half);
  const std::size_t half_bytes_inter_full = static_cast<std::size_t>(inter_full_) * sizeof(__half);
  ...
  const std::size_t cache_bytes = static_cast<std::size_t>(n_layers_) *
                                  static_cast<std::size_t>(max_ctx_) *
                                  static_cast<std::size_t>(kv_dim_) * sizeof(__half);
  const std::size_t rope_bytes =
      static_cast<std::size_t>(max_ctx_) * static_cast<std::size_t>(head_dim_ / 2) * sizeof(float);
  const std::size_t expert_gate_up_bytes =
      static_cast<std::size_t>(hidden_) * static_cast<std::size_t>(inter_full_) * sizeof(__half);
  const std::size_t expert_down_bytes =
      static_cast<std::size_t>(inter_expert_) * static_cast<std::size_t>(hidden_) * sizeof(__half);
  const std::size_t streamed_rowmajor_bytes =
      std::max(static_cast<std::size_t>(inter_shared_) * static_cast<std::size_t>(hidden_),
               static_cast<std::size_t>(hidden_) * static_cast<std::size_t>(inter_shared_)) *
      sizeof(__half);
  ...
  malloc_device(&d_ff_inter_, std::max(half_bytes_inter_expert, half_bytes_inter_shared));
  malloc_device(&d_shared_gate_out_, half_bytes_inter_shared);
  malloc_device(&d_shared_up_out_, half_bytes_inter_shared);
  malloc_device(&d_logits_, static_cast<std::size_t>(vocab_size_) * sizeof(float));
  ...
  bf16_stage_elems_ = static_cast<std::size_t>(hidden_) * static_cast<std::size_t>(inter_full_);
...
void Llama4CudaEngine::copy_bf16_tensor_to_fp16_device(const std::uint16_t* src, void* dst,
                                                       std::size_t elems) {
  ...
  while (offset < elems) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_bf16_stage_, src + offset, chunk * sizeof(std::uint16_t),
                               cudaMemcpyHostToDevice, compute_stream_));
    kernels::launch_convert_bf16_to_fp16(d_bf16_stage_, dst_half + offset, static_cast<int>(chunk),
                                         compute_stream_);
    offset += chunk;
  }
}
...
void Llama4CudaEngine::load_resident_weights() {
  ...
  alloc_and_copy(&d_lm_head_,
                 load_tensor(lm_head_name, static_cast<std::size_t>(vocab_size_) *
                                               static_cast<std::size_t>(hidden_)),
                 static_cast<std::size_t>(vocab_size_) * static_cast<std::size_t>(hidden_));

  std::vector<std::uint16_t> unit_norm(static_cast<std::size_t>(head_dim_),
                                       float_to_half_bits(1.0f));
  CUDA_CHECK(cudaMalloc(&d_q_norm_unit_, static_cast<std::size_t>(head_dim_) * sizeof(__half)));
  CUDA_CHECK(cudaMalloc(&d_k_norm_unit_, static_cast<std::size_t>(head_dim_) * sizeof(__half)));
  CUDA_CHECK(cudaMemcpyAsync(d_q_norm_unit_, unit_norm.data(),
                             static_cast<std::size_t>(head_dim_) * sizeof(std::uint16_t),
                             cudaMemcpyHostToDevice, compute_stream_));
  CUDA_CHECK(cudaMemcpyAsync(d_k_norm_unit_, unit_norm.data(),
                             static_cast<std::size_t>(head_dim_) * sizeof(std::uint16_t),
                             cudaMemcpyHostToDevice, compute_stream_));
  ...
  const std::size_t shared_fp16_per_layer = 3ULL * static_cast<std::size_t>(inter_shared_) *
  ...
  const std::size_t vram_for_shared =
      (free_vram > attn_total + free_vram / 20) ? (free_vram - attn_total - free_vram / 20) : 0;
  n_shared_gpu_ = static_cast<int>(
      std::min(static_cast<std::size_t>(n_layers_), vram_for_shared / shared_fp16_per_layer));
  ...
  for (int layer = 0; layer < n_layers_; ++layer) {
    ...
    alloc_and_copy(
        &dev.norm_att,
        load_tensor(prefix + ".input_layernorm.weight", static_cast<std::size_t>(hidden_)),
        static_cast<std::size_t>(hidden_));
    alloc_and_copy(
        &dev.norm_ffn,
        load_tensor(prefix + ".post_attention_layernorm.weight", static_cast<std::size_t>(hidden_)),
        static_cast<std::size_t>(hidden_));
    alloc_and_copy(
        &dev.wq,
        load_tensor(prefix + ".self_attn.q_proj.weight",
                    static_cast<std::size_t>(hidden_) * static_cast<std::size_t>(hidden_)),
        static_cast<std::size_t>(hidden_) * static_cast<std::size_t>(hidden_));
    alloc_and_copy(
        &dev.wk,
        load_tensor(prefix + ".self_attn.k_proj.weight",
                    static_cast<std::size_t>(kv_dim_) * static_cast<std::size_t>(hidden_)),
        static_cast<std::size_t>(kv_dim_) * static_cast<std::size_t>(hidden_));
    alloc_and_copy(
        &dev.wv,
    ...
    if (weights_.has_tensor(q_norm_name)) {
      alloc_and_copy(&dev.q_norm, load_tensor(q_norm_name, static_cast<std::size_t>(head_dim_)),
    ...
    if (weights_.has_tensor(k_norm_name)) {
      alloc_and_copy(&dev.k_norm, load_tensor(k_norm_name, static_cast<std::size_t>(head_dim_)),
                     static_cast<std::size_t>(head_dim_));
    }

    host.router =
        load_tensor(prefix + ".feed_forward.router.weight",
                    static_cast<std::size_t>(n_experts_) * static_cast<std::size_t>(hidden_));
    host.gate_up =
        load_tensor(prefix + ".feed_forward.experts.gate_up_proj",
                    static_cast<std::size_t>(n_experts_) * static_cast<std::size_t>(hidden_) *
                        static_cast<std::size_t>(inter_full_));
    host.down_exp =
        load_tensor(prefix + ".feed_forward.experts.down_proj",
                    static_cast<std::size_t>(n_experts_) * static_cast<std::size_t>(inter_expert_) *
                        static_cast<std::size_t>(hidden_));
    ...
    if (layer < n_shared_gpu_) {
      alloc_and_copy(&dev_sh.sh_gate, sh_gate_src,
                     static_cast<std::size_t>(inter_shared_) * static_cast<std::size_t>(hidden_));
...
void Llama4CudaEngine::build_rope_tables() {
  ...
  for (int d = 0; d < half_dim; ++d) {
    ...
    for (int p = 0; p < max_ctx_; ++p) {
      ...
      rope_cos[static_cast<std::size_t>(p) * static_cast<std::size_t>(half_dim) +
               static_cast<std::size_t>(d)] = std::cos(angle);
      rope_sin[static_cast<std::size_t>(p) * static_cast<std::size_t>(half_dim) +
...
void Llama4CudaEngine::load_token_embedding_to_device(int token) {
  ...
  const std::uint16_t* row =
      h_tok_embeddings_bf16_ + static_cast<std::size_t>(token) * static_cast<std::size_t>(hidden_);
  ...
  CUDA_CHECK(cudaMemcpyAsync(d_x_, h_token_embedding_fp16_.data(),
                             static_cast<std::size_t>(hidden_) * sizeof(std::uint16_t),
                             cudaMemcpyHostToDevice, compute_stream_));
}

int Llama4CudaEngine::select_expert_cpu(int layer) {
  CUDA_CHECK(cudaStreamSynchronize(compute_stream_));
  CUDA_CHECK(cudaMemcpy(h_x_norm_fp16_.data(), d_x_norm_,
                        static_cast<std::size_t>(hidden_) * sizeof(std::uint16_t),
  ...
  const std::uint16_t* router = layer_host_moe_[static_cast<std::size_t>(layer)].router;
  int best_idx = 0;
  float best_score = -std::numeric_limits<float>::infinity();
  for (int expert = 0; expert < n_experts_; ++expert) {
    const std::uint16_t* row =
        router + static_cast<std::size_t>(expert) * static_cast<std::size_t>(hidden_);
    float sum = 0.0f;
    for (int i = 0; i < hidden_; ++i) {
...
void Llama4CudaEngine::load_layer_moe_weights_to_device(int layer, int expert_idx) {
  ...
  const std::uint16_t* gate_up = host.gate_up + static_cast<std::size_t>(expert_idx) *
                                                    static_cast<std::size_t>(hidden_) *
                                                    static_cast<std::size_t>(inter_full_);
  const std::uint16_t* down = host.down_exp + static_cast<std::size_t>(expert_idx) *
                                                  static_cast<std::size_t>(inter_expert_) *
                                                  static_cast<std::size_t>(hidden_);

  copy_bf16_tensor_to_fp16_device(
      gate_up, d_expert_gate_up_w_,
      static_cast<std::size_t>(hidden_) * static_cast<std::size_t>(inter_full_));
  copy_bf16_tensor_to_fp16_device(
      down, d_expert_down_w_,
      static_cast<std::size_t>(inter_expert_) * static_cast<std::size_t>(hidden_));
  ++last_benchmark_stats_.streamed_layer_copies;
}

void Llama4CudaEngine::rowmajor_projection_half(const void* w, const void* x, void* y,
...
void Llama4CudaEngine::rowmajor_projection_float(const void* w, const void* x, void* y,
                                                 int out_features, int in_features) {
...
void Llama4CudaEngine::transposed_projection_half(const void* w_in_out, const void* x, void* y,
                                                  int in_features, int out_features) {
  constexpr float alpha = 1.0f;
  constexpr float beta = 0.0f;
  CUBLAS_CHECK(cublasGemmEx(cublas_, CUBLAS_OP_N, CUBLAS_OP_N, out_features, 1, in_features, &alpha,
                            w_in_out, CUDA_R_16F, out_features, x, CUDA_R_16F, in_features, &beta,
                            y, CUDA_R_16F, out_features, CUBLAS_COMPUTE_32F,
...
void Llama4CudaEngine::initialize(const EngineOptions& options) {
  ...
  if (options_.max_context > 0) {
    max_ctx_ = std::min(options_.max_context, rope_orig_max_pos_ * static_cast<int>(rope_scale_));
  }
  ...
  try {
    ...
    if (options_.verbose) {
      ...
      const long long shared_mb =
          static_cast<long long>(n_layers_) * 3LL * inter_shared_ * hidden_ * 2 / (1024 * 1024);
      const long long attn_mb = static_cast<long long>(n_layers_) *
                                (static_cast<long long>(hidden_) * hidden_ * 2LL +
                                 static_cast<long long>(kv_dim_) * hidden_ * 4LL) *
                                2 / (1024 * 1024);
      const long long lm_head_mb =
          static_cast<long long>(vocab_size_) * hidden_ * 2 / (1024 * 1024);
      std::cout << "[llama4_cuda] layers=" << n_layers_ << " hidden=" << hidden_
                << " heads=" << n_heads_ << "/" << n_kv_heads_ << " experts=" << n_experts_
                << " vocab=" << vocab_size_ << " max_ctx=" << max_ctx_ << " kv_dim=" << kv_dim_
