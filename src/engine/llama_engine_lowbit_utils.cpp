...
constexpr std::array<const char*, 3> kMlpWeightSuffixes = {".feed_forward.w1", ".feed_forward.w2",
                                                           ".feed_forward.w3"};
...
bool has_lowbit_source_tensor(const model::WeightLoader& weights, const std::string& name,
                              int quant_bits) {
  ...
  for (int row = 0; row < rows; ++row) {
    ...
    for (int col = 0; col < cols; ++col) {
      const std::uint8_t byte =
          static_cast<std::uint8_t>(src[src_row + static_cast<std::size_t>(col / 2)]);
      const std::uint8_t nibble = (col & 1) == 0 ? (byte & 0x0Fu) : ((byte >> 4) & 0x0Fu);
      const std::int8_t signed_q =
          (nibble >= 8) ? static_cast<std::int8_t>(nibble) - 16 : static_cast<std::int8_t>(nibble);
  ...
  for (int row = 0; row < rows; ++row) {
    ...
    for (int col = 0; col < cols; col += 2) {
      ...
      const std::uint8_t lo =
          clamp_i4(static_cast<int>(src[src_row + static_cast<std::size_t>(col)]));
      std::uint8_t hi = 0;
      if (col + 1 < cols) {
