...
void Llama4CudaEngine::forward_token(int token, int position, bool compute_logits,
  ...
  for (int layer = 0; layer < n_layers_; ++layer) {
    ...
    time_or_launch(&last_benchmark_stats_.decode_rmsnorm_ms, [&] {
      kernels::launch_rmsnorm(
          static_cast<const __half*>(d_x_), static_cast<const __half*>(weights.norm_att),
          static_cast<__half*>(d_x_norm_), 1, hidden_, kRmsNormEps, compute_stream_);
    });
    ...
    if (use_qk_norm_) {
      kernels::launch_rmsnorm(
          ...
          static_cast<const __half*>(weights.q_norm ? weights.q_norm : d_q_norm_unit_),
          static_cast<__half*>(d_q_), n_heads_, head_dim_, kRmsNormEps, compute_stream_);
      kernels::launch_rmsnorm(
          static_cast<const __half*>(d_k_),
          static_cast<const __half*>(weights.k_norm ? weights.k_norm : d_k_norm_unit_),
          static_cast<__half*>(d_k_), n_kv_heads_, head_dim_, kRmsNormEps, compute_stream_);
    }

    kernels::launch_rope_inplace_table(static_cast<__half*>(d_q_), static_cast<__half*>(d_k_),
                                       n_heads_, n_kv_heads_, head_dim_, position,
                                       static_cast<const float*>(d_rope_cos_),
                                       static_cast<const float*>(d_rope_sin_), compute_stream_);

    time_or_launch(&last_benchmark_stats_.decode_kv_store_ms, [&] {
      CUDA_CHECK(cudaMemcpyAsync(
          k_cache_layer + static_cast<std::size_t>(position) * static_cast<std::size_t>(kv_dim_),
          d_k_, static_cast<std::size_t>(kv_dim_) * sizeof(__half), cudaMemcpyDeviceToDevice,
          compute_stream_));
      CUDA_CHECK(cudaMemcpyAsync(
          v_cache_layer + static_cast<std::size_t>(position) * static_cast<std::size_t>(kv_dim_),
          d_v_, static_cast<std::size_t>(kv_dim_) * sizeof(__half), cudaMemcpyDeviceToDevice,
          compute_stream_));
    });
    ...
    time_or_launch(&last_benchmark_stats_.decode_attention_ms, [&] {
      kernels::launch_attention_step(static_cast<const __half*>(d_q_), k_cache_layer, v_cache_layer,
                                     static_cast<__half*>(d_att_), position + 1, n_heads_,
                                     n_kv_heads_, head_dim_, compute_stream_);
    });
    ...
    time_or_launch(&last_benchmark_stats_.decode_wo_ms, [&] {
      ...
      kernels::launch_add_inplace(static_cast<__half*>(d_x_),
                                  static_cast<const __half*>(d_tmp_hidden_), hidden_,
                                  compute_stream_);
    });

    time_or_launch(&last_benchmark_stats_.decode_rmsnorm_ms, [&] {
      kernels::launch_rmsnorm(
          static_cast<const __half*>(d_x_), static_cast<const __half*>(weights.norm_ffn),
          static_cast<__half*>(d_x_norm_), 1, hidden_, kRmsNormEps, compute_stream_);
    });
    ...
    if (layer + 1 < n_layers_) {
      ...
      if (layer_device_shared_[static_cast<std::size_t>(next)].sh_gate == nullptr) {
        ...
        prefetch_range(nh.router, static_cast<std::size_t>(n_experts_) *
    ...
    if (options_.profile_decode_phases) {
      last_benchmark_stats_.transfer_ms += measure_stream_ms(
          compute_stream_, [&] { load_layer_moe_weights_to_device(layer, expert_idx); });
    } else {
    ...
    auto run_mlp = [&] {
      ...
      transposed_projection_half(d_expert_gate_up_w_, d_x_norm_, d_ff13_, hidden_, inter_full_);
      kernels::launch_silu_mul(static_cast<const __half*>(d_ff13_),
                               static_cast<const __half*>(d_ff13_) + inter_expert_,
                               static_cast<__half*>(d_ff_inter_), inter_expert_, compute_stream_);
      transposed_projection_half(d_expert_down_w_, d_ff_inter_, d_tmp_hidden_, inter_expert_,
                                 hidden_);

      // Shared expert FFN.
      if (dev_sh.sh_gate != nullptr) {
        // GPU-resident path: pure compute, no disk I/O.
        rowmajor_projection_half(dev_sh.sh_gate, d_x_norm_, d_shared_gate_out_, inter_shared_,
                                 hidden_);
        rowmajor_projection_half(dev_sh.sh_up, d_x_norm_, d_shared_up_out_, inter_shared_, hidden_);
        kernels::launch_silu_mul(static_cast<const __half*>(d_shared_gate_out_),
                                 static_cast<const __half*>(d_shared_up_out_),
                                 static_cast<__half*>(d_ff_inter_), inter_shared_, compute_stream_);
        rowmajor_projection_half(dev_sh.sh_down, d_ff_inter_, d_att_, hidden_, inter_shared_);
      } else {
        ...
        copy_bf16_tensor_to_fp16_device(
            ...
            static_cast<std::size_t>(inter_shared_) * static_cast<std::size_t>(hidden_));
        rowmajor_projection_half(d_streamed_rowmajor_w_, d_x_norm_, d_shared_gate_out_,
                                 inter_shared_, hidden_);
        copy_bf16_tensor_to_fp16_device(
            host.sh_up, d_streamed_rowmajor_w_,
            static_cast<std::size_t>(inter_shared_) * static_cast<std::size_t>(hidden_));
        rowmajor_projection_half(d_streamed_rowmajor_w_, d_x_norm_, d_shared_up_out_, inter_shared_,
                                 hidden_);
        kernels::launch_silu_mul(static_cast<const __half*>(d_shared_gate_out_),
                                 static_cast<const __half*>(d_shared_up_out_),
                                 static_cast<__half*>(d_ff_inter_), inter_shared_, compute_stream_);
        ...
        rowmajor_projection_half(d_streamed_rowmajor_w_, d_ff_inter_, d_att_, hidden_,
                                 inter_shared_);
      }
      ...
      kernels::launch_add_inplace(static_cast<__half*>(d_x_),
                                  static_cast<const __half*>(d_tmp_hidden_), hidden_,
                                  compute_stream_);
      kernels::launch_add_inplace(static_cast<__half*>(d_x_), static_cast<const __half*>(d_att_),
  ...
  time_or_launch(&last_benchmark_stats_.decode_lm_head_ms, [&] {
    ...
    if (out_argmax) {
      kernels::launch_argmax_float(static_cast<const float*>(d_logits_), vocab_size_, d_argmax_,
                                   compute_stream_);
    }
  });
...
void Llama4CudaEngine::forward_token_logits(int token, int position, std::vector<float>* out_logits,
                                            int* out_argmax) {
  forward_token(token, position, true, out_logits, out_argmax);
}

int Llama4CudaEngine::sample_next_token(float temperature, const std::vector<int>& history) {
  ...
  return sample_from_logits(h_logits_, temperature, options_.top_k, options_.top_p,
                            options_.repetition_penalty, options_.no_repeat_ngram_size, history);
}
...
std::vector<int> Llama4CudaEngine::generate_stream(const std::vector<int>& prompt_tokens,
                                                   int max_new_tokens, float temperature,
...
std::vector<std::pair<int, float>> Llama4CudaEngine::inspect_next_logits(
  ...
  if (prompt_tokens.empty()) {
    ...
    forward_token_logits(prompt_tokens.back(), static_cast<int>(prompt_tokens.size()) - 1, nullptr,
                         nullptr);
  }
