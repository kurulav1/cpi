...
int main() {
  ...
  std::vector<half> kp(static_cast<std::size_t>(num_pool_blocks) * block_size * kv_hidden, __float2half(0.0f));
  ...
  for (int c = 0; c < num_chunks; ++c) {
    ...
    for (int local = 0; local < clen; ++local) {
      ...
      for (int d = 0; d < kv_hidden; ++d) {
        kp[static_cast<std::size_t>(phys) * kv_hidden + d] = kf[static_cast<std::size_t>(t) * kv_hidden + d];
        vp[static_cast<std::size_t>(phys) * kv_hidden + d] = vf[static_cast<std::size_t>(t) * kv_hidden + d];
      }
    }
  }
  ...
  CK(cudaMalloc(&so, static_cast<std::size_t>(num_heads) * scratch_chunks * head_dim * sizeof(float)));
  ...
  kernels::launch_attention_step(dq, dkf, dvf, dref, seq_len, num_heads, num_kv_heads,
                                 head_dim, 0, sm, sl, so, scratch_chunks, true);
  // Paged: same values via a non-contiguous block table.
  kernels::launch_attention_step_paged(dq, dkp, dvp, dbt, dpaged, seq_len, num_heads,
  ...
  std::printf("paged_attention_parity: seq_len=%d heads=%d kv=%d head_dim=%d block=%d\n",
              seq_len, num_heads, num_kv_heads, head_dim, block_size);
  std::printf("  non-contiguous block table (reversed); max_abs_diff=%.3e mismatched_elems=%d/%zu\n",
