...
int main() {
  ...
  int next_phys =
      pool_blocks - 1;  // hand out physical blocks top-down -> non-contiguous vs logical
  ...
  CK(cudaMalloc(&so, static_cast<std::size_t>(batch) * num_heads * scratch_chunks * head_dim *
                         sizeof(float)));
  ...
  for (int b = 0; b < batch; ++b) {
    ...
    kernels::launch_attention_step_paged(qb, dkp, dvp, btb, dout_ref, seq_lens[b], num_heads,
                                         num_kv_heads, head_dim, block_size, 0, sm, sl, so,
                                         scratch_chunks);
    CK(cudaDeviceSynchronize());
    ...
    for (std::size_t i = 0; i < ref.size(); ++i) {
      const double d =
          std::fabs(static_cast<double>(__half2float(ref[i])) -
                    static_cast<double>(__half2float(
                        out_batched[static_cast<std::size_t>(b) * num_heads * head_dim + i])));
      max_abs = std::max(max_abs, d);
    }
  }

  std::printf(
      "paged_batched_attention_parity: batch=%d seq_lens={40,100,200} heads=%d kv=%d hd=%d\n",
      batch, num_heads, num_kv_heads, head_dim);
  std::printf(
      "  per-seq non-contiguous block tables, shared pool; max_abs_diff(batched vs single)=%.3e\n",
      max_abs);
