// Verifies the C++ YARN rope frequencies (engine/deepseek_rope.hpp) reproduce transformers'
// inv_freq for DeepSeek-V2-Lite. The reference (artifacts/hf_inv_freq.f32, 32 float32 dumped from
// the model's rotary) is generated out-of-band; the test skips cleanly if absent. This is the rope
// prerequisite for the in-engine MLA op; getting inv_freq wrong is a silent, fluent-garbage
// failure, so it is pinned to the real values here.
