...
// host oracle. Isolated, model-blocked groundwork for Kimi K3 (whose checkpoint doesn't fit this
// box). CPI already runs the Qwen3.5 gated delta-net with a per-head SCALAR decay (verified). KDA's
// distinction is FINE-GRAINED gating: a decay vector over the key channels, so each key dimension
// of the recurrent state forgets at its own rate. This brick implements that per-channel gated
// delta rule as a standalone recurrence and checks it against a plain-C++ oracle.
//
...
// (The QK-norm and the RMS/SiLU output gate CPI applies around this are orthogonal,
// separately-tested stages; this brick isolates the recurrence, which is the KDA-specific part.)
